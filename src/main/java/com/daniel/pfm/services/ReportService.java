...
public class ReportService {
    ...
    public SummaryResponseDTO getMonthlySummary(String username, String month) {
        ...
        BigDecimal totalIncome = repository.sumByUserAndUserAndTypeAndDataBetween(
                user,
        ...
        BigDecimal totalExpense = repository.sumByUserAndUserAndTypeAndDataBetween(
                user,
    ...
    public List<CategorySummaryDTO> getCategorySummary(String username, String month, TransactionType type) {
        ...
        List<Object[]> results = repository.sumByCategoryAndUserAndTypeAndDataBetween(
                user,
                type,
                start,
                end
        ...
        return results.stream()
                .map(row -> {
                    ...
                    BigDecimal percentage = total.divide(
                            totalGeral,
                            4,
                            RoundingMode.HALF_UP)
                            .multiply(BigDecimal.valueOf(100));
                    return new CategorySummaryDTO(category.getId(), category.getName(), total, percentage);
                }).toList();
